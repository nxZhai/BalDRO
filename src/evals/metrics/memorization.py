...
@unlearning_metric(name="hm_aggregate_inverted")
def hm_aggregate_inverted(model, **kwargs):
    values = [1 - result["agg_value"] for _, result in kwargs["pre_compute"].items()]
    return {"agg_value": sc.stats.hmean(values)}


#   - .@pre_compute.forget_Q_A_Prob: forget_Q_A_Prob
#   - .@pre_compute.forget_Truth_Ratio: forget_Truth_Ratio
#   - .@pre_compute.forget_Q_A_ROUGE: forget_Q_A_ROUGE


@unlearning_metric(name="forget_success")
def forget_success(model, **kwargs):
    forget_Q_A_Prob = kwargs["pre_compute"]["forget_Q_A_Prob"]["agg_value"]
    forget_Q_A_ROUGE = kwargs["pre_compute"]["forget_Q_A_ROUGE"]["agg_value"]
    forget_Truth_Ratio = kwargs["pre_compute"]["forget_Truth_Ratio"]["agg_value"]

    values = [1 - forget_Q_A_Prob, 1 - forget_Q_A_ROUGE, forget_Truth_Ratio]
    return {"agg_value": sc.stats.hmean(values)}
